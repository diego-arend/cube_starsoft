...
# docker-up-ngrok.sh
#
# Phased startup for the ngrok stack:
#
...
#
# Usage:
#   pnpm run docker:up:ngrok          (called via package.json)
#   bash scripts/docker-up-ngrok.sh   (directly)

set -euo pipefail

COMPOSE_FILE="docker/ngrok/docker-compose-ngrok.yml"
COMPOSE="docker compose -f $COMPOSE_FILE"

# ── Phase 1: Infrastructure ──────────────────────────────────────────────────
echo "[ngrok] ── Phase 1: Building images and starting infrastructure..."
$COMPOSE up -d --build \
  postgres redis rabbitmq \
  traefik mailpit minio pgadmin redisinsight ngrok
...
echo "[ngrok] Waiting for infrastructure healthchecks..."
for svc in turborepo_saas_postgres_ngrok turborepo_saas_redis_ngrok turborepo_saas_rabbitmq_ngrok; do
  echo "[ngrok]   waiting for $svc to be healthy..."
  until [ "$(docker inspect --format='{{.State.Health.Status}}' "$svc" 2>/dev/null)" = "healthy" ]; do
    sleep 3
  done
  echo "[ngrok]   $svc is healthy ✔"
done

# ── Phase 2: Migration ───────────────────────────────────────────────────────
echo "[ngrok] ── Phase 2: Running database migrations..."
...
if [ "$MIGRATION_EXIT" != "0" ]; then
  echo "[ngrok] ERROR: Migration exited with code $MIGRATION_EXIT. Aborting."
  $COMPOSE logs migration
  exit 1
fi

echo "[ngrok] Migrations completed successfully. Removing migration container..."
$COMPOSE rm -f migration
echo "[ngrok] Migration container removed."

# ── Phase 3: Application services ────────────────────────────────────────────
echo "[ngrok] ── Phase 3: Starting application services..."
...
echo "[ngrok] Waiting for backend to become healthy..."
