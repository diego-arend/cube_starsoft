...
import { KnowledgeBaseService } from "../types";
...
export class AssistantGraph {
  ...
  private knowledgeBaseService?: KnowledgeBaseService; // optional RAG provider
  ...
  constructor(
    ...
    // systemMessage will be set dynamically via setSystemMessage
...
9. CONSISTÊNCIA E VERDADE: O CONTEXTO PRIORITÁRIO (RAG) é a sua fonte da verdade. Ignore o histórico se ele conflitar com as informações novas do RAG.
  ...
  private async retrievalNode(state: AssistantState, config: RunnableConfig) {
    ...
    if (typeof lastMessage.content === "string") {
      ...
      const textPart = (lastMessage.content as ContentPart[]).find(
        (p) => p.type === "text"
      );
      query = (textPart as ContentPart | undefined)?.text ?? "";
    }
